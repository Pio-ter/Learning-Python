...
print('Najwyższa wartość procentowa: ', max(percent))
print('Najniższa wartość procentowa: ', min(percent))

i = 0

for i in range(0, len(countries)):
    print(countries[i], ':', percent[i], ', procent zdobytych punktów:', int(percent[i]), ', liczba zdobytych punktów:', int(round(sumOfPoints*percent[i]/100)), round(percent[i],2))
    i+=1
